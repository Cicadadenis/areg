...
def mult_buy(userid,reqws,phone):
	text = f'''<b>❤️ Сервисы: <code>{reqws}</code>
📱 Ваш номер: <code>{phone}</code>

Если смс не прийдет через 5 минут, то вам вернуться потраченные деньги за этот номер, аренда номера будет отменена!!!</b>'''
	return text
...
def admin_stata():
	...
	print(count_users)
	q.execute(f'SELECT COUNT(userid) FROM ugc_users where date_reg LIKE "{time}%"')
	count_users_24 = q.fetchone()[0]
	q.execute(f'SELECT COUNT(userid) FROM ugc_users where date_reg LIKE "{time_two}%"')
	count_users_1 = q.fetchone()[0]
	q.execute(f'SELECT SUM(summa) FROM ugc_pays where date_pay LIKE  "{time}%"')
	statistika_qiwi_24 = q.fetchone()[0]
	q.execute(f'SELECT SUM(summa) FROM ugc_pays_btc where date_pay LIKE "{time}%"')
	statistika_btc_24 = q.fetchone()[0]
	q.execute(f'SELECT SUM(summa) FROM ugc_pays')
	statistika_qiwi = q.fetchone()[0]
	q.execute(f'SELECT SUM(summa) FROM ugc_pays_btc')
	statistika_btc = q.fetchone()[0]
